    ...
    <>
      <section className='max-w-[600px] mt-10 w-full mx-auto px-3'>
        <div className="mb-6 relative">
          <Input type="text" value={value} onChange={(e) => handleChange(e)} id="default-input" className=" w-full !p-3 h-[3rem]" placeholder='Search Skill Here...!' />
          {
            value && <ul className='bg-gray-100  w-full p-5 mt-1 rounded-lg max-h-80 overflow-x-hidden overflow-scroll'>
              {
        ...
        <div>
          ...
          <ul>
              ...
              <SortableContext items={selectedSkill} strategy={verticalListSortingStrategy}>
                {
      ...
      {/*  Notifaction Toast  */}
      <section className={`fixed transition-all duration-400 ease-in-out top-4 ${isExistSkill?.state ? 'right-4' : '-right-[100%]'} `}>
...
const SkillList = ({ s, removeSkill }) => {
  ...
  return (
    <li className="relative">
        <h6 ref={setNodeRef} {...attributes} {...listeners} style={style} className='bg-gray-100 z-10 border p-3 mb-3 border-gray-200 rounded-lg flex  justify-between items-center'><p>{s.skill} - <span className='text-gray-400 text-sm'>{s.level} </span></p></h6>
        <button ref={setNodeRef}  style={style} className='text-xl cursor-pointer absolute z-10 right-3 top-4 hover:scale-110' onClick={() => removeSkill(s)}><RxCross2 /></button>
      </li>
  )
}